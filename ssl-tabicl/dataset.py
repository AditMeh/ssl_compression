...
def get_satellite_train_embeddings(percent=100, root_dir="embeddings/satellite", train_percent=80, seed=42):
    """
    Returns a tuple (embeddings, labels) for satellite train split.
    Constructs train/test splits from a single folder based on a percentage, with shuffling and a fixed seed.
    Allows selecting a percentage of embeddings from the training set.
    
    Args:
        percent (float): Percentage of training embeddings to load (1-100)
        root_dir (str): Path to embeddings root directory containing class folders
        train_percent (float): Percentage of data to use for training (remaining goes to test) (1-99)
        seed (int): Random seed for shuffling (default: 42)
    ...
    Example:
        If train_percent=80, then 80% of data goes to train and 20% to test.
    ...
    n_total = len(all_embeddings)
    n_train = int(np.ceil((train_percent / 100.0) * n_total))
    
    embeddings = all_embeddings[:n_train]
    labels = all_labels[:n_train]
    
    # Apply percent reduction if specified
    if percent < 100:
        n_use = int(np.ceil((percent / 100.0) * len(embeddings)))
        embeddings = embeddings[:n_use]
        labels = labels[:n_use]
...
def get_satellite_test_embeddings(root_dir="embeddings/satellite", train_percent=80, seed=42):
    """
    Returns (embeddings, labels) for satellite test split.
    Constructs train/test splits from a single folder based on a percentage, with shuffling and a fixed seed.
